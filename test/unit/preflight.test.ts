...
`,
		...
		writeAgent(
			path.join(tempDir, "home", ".pi", "agent", "agents", "worker.md"),
			`---
name: worker
...
`,
		...
		try {
			...
			assert.ok(
				result.contract.agent.shadowedCandidates.some(
					(candidate) =>
						candidate.name === "worker" && candidate.source === "user",
				),
			);
			...
			assert.deepEqual(result.contract.tools.capabilityAudit?.removedTools, [
				"write",
			]);
			assert.equal(
				result.contract.tools.capabilityAudit?.removedExtensionCount,
				1,
			);
			assert.equal(result.contract.tools.disableAmbientExtensions, true);
			assert.equal(
				result.contract.roots.sessionFile,
				path.join(sessionRoot, "run-123", "run-0", "session.jsonl"),
			);
			assert.equal(
				result.contract.roots.outputPath,
				path.join(
					cwd,
					".pi-subagents",
					"artifacts",
			...
			assert.equal(
				result.contract.roots.lifecycle?.statusPath.endsWith(
					path.join("run-123", "status.json"),
				),
				true,
			);
			assert.equal(
				result.contract.roots.lifecycle?.eventsPath.endsWith(
					path.join("run-123", "events.jsonl"),
				),
				true,
			);
			assert.equal(
				result.contract.roots.lifecycle?.processTerminalPath.endsWith(
					path.join("run-123", "process-terminal.json"),
				),
				true,
			);
			assert.notEqual(
				result.contract.roots.lifecycle?.asyncDir,
				result.contract.roots.artifactsDir,
			);
...
`,
		...
		assert.match(
			result.message,
			/Ambiguous agent alias 'coder': reviewer, worker|Ambiguous agent alias 'coder': worker, reviewer/,
		);
	});
...
`,
		...
		assert.notEqual(
			after.contract.agent.definitionDigest,
			before.contract.agent.definitionDigest,
		);
		assert.notEqual(
			after.contract.launchContractDigest,
			before.contract.launchContractDigest,
		);
...
`,
		...
		const before = await resolveSubagentLaunchContract({
			agent: "worker",
			cwd,
			runId: "skill-digest-test",
		});
		assert.equal(before.ok, true);
		fs.writeFileSync(
			path.join(cwd, ".pi", "skills", "digest-skill", "SKILL.md"),
...
`,
		...
		const missingAgent = await resolveSubagentLaunchContract({
			agent: "missing",
			cwd,
		});
		assert.deepEqual(missingAgent, {
			ok: false,
			code: "missing_agent",
		...
		const missingSkill = await resolveSubagentLaunchContract({
			agent: "worker",
			cwd,
		});
...
`,
		...
		const invalidCwd = await resolveSubagentLaunchContract({
			agent: "worker",
		...
		const unsupportedMode = await resolveSubagentLaunchContract({
			agent: "worker",
			cwd,
		...
		const invalidArtifactDir = await resolveSubagentLaunchContract({
			agent: "worker",
			cwd,
			artifactDir: "bogus" as never,
		});
	...
	it("projects MCP, extension, fanout, structured-output, and fork diagnostics", async () => {
		...
		writeMcpFixture();
		writeAgent(
			path.join(cwd, ".pi", "agents", "fanout.md"),
			`---
name: fanout
...
`,
		...
		assert.ok(
			result.contract.diagnostics.some(
				(diagnostic) => diagnostic.code === "host_required",
			),
		);
		assert.deepEqual(result.contract.tools.declaredBuiltin, [
			"read",
			"subagent",
		]);
		assert.equal(result.contract.tools.explicitAllowlist, true);
		assert.equal(result.contract.tools.fanoutAuthorized, true);
		assert.deepEqual(result.contract.tools.internalTools, [
			"structured_output",
		]);
		assert.deepEqual(result.contract.tools.effectiveMcpTools, [
		...
		assert.deepEqual(result.contract.tools.toolExtensionPaths, [
			"/tmp/tool-ext.ts",
		]);
		assert.equal(result.contract.tools.disableAmbientExtensions, true);
		assert.ok(
			result.contract.tools.runtimeExtensions.some((extensionPath) =>
				extensionPath.endsWith("subagent-prompt-runtime.ts"),
			),
		);
		assert.ok(
			result.contract.tools.runtimeExtensions.some((extensionPath) =>
				extensionPath.endsWith("fanout-child.ts"),
			),
		);
		assert.ok(
			result.contract.tools.extensionArgs.includes("/tmp/config-ext.ts"),
		);
		assert.ok(
			result.contract.tools.extensionArgs.includes("/tmp/subagent-only.ts"),
		);
	});
