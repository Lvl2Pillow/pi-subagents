...
function writeUserAgent(name: string): void {
	fs.mkdirSync(path.dirname(userAgentPath(name)), { recursive: true });
	fs.writeFileSync(
		userAgentPath(name),
...
describe("agent eject/disable/enable/reset management actions", () => {
	...
	describe("eject", () => {
		it("ejects to project scope when agentScope is project", () => {
			...
			assert.equal(
				discoverAgentsAll(tempDir).project.find((a) => a.name === "scout")
					?.source,
				"project",
			);
		});
		...
		it("copies a package agent that shadows a same-named agent by runtime precedence", () => {
			...
			assert.equal(
				fs.readFileSync(userAgentPath("reviewer"), "utf-8"),
				fs.readFileSync(packageAgentPath("reviewer"), "utf-8"),
			);
		});
		...
		it("refuses to eject an unknown agent", () => {
			...
			assert.match(
				readText(ejected),
				/not found or is not a bundled\/package agent/,
			);
		});
	});
	...
	describe("disable", () => {
		it("merges disabled into an existing override without dropping other fields", () => {
			...
			const settings = readJson(userSettingsPath()) as {
				subagents: {
					agentOverrides: { reviewer: { model: string; disabled: boolean } };
				};
			};
		...
		it("writes a project-scoped override when agentScope is project", () => {
			...
			const all = discoverAgentsAll(tempDir).user.find(
				(a) => a.name === "reviewer",
			);
		...
		it("also disables a custom agent via a settings override", () => {
			...
			assert.equal(disabled.isError, false);
			assert.equal(
				discoverAgents(tempDir, "both").agents.find((a) => a.name === "helper"),
				undefined,
			);
		});
		...
		it("also disables a package agent via a settings override", () => {
			...
			const all = discoverAgentsAll(tempDir).package.find(
				(a) => a.name === "packaged-reviewer",
			);
	...
	describe("enable", () => {
		...
		it("preserves other override fields when removing the disabled flag", () => {
			...
			writeJson(userSettingsPath(), {
				subagents: {
					agentOverrides: {
						reviewer: { model: "openai/gpt-5.4", disabled: true },
					},
				},
			});
		...
		it("points to the disabling scope when enabling the wrong scope", () => {
			...
			assert.match(
				readText(enabled),
				/still disabled via a project scope override/,
			);
	...
	describe("reset", () => {
		it("removes a settings override and restores the package default", () => {
			...
			assert.equal(
				(readJson(userSettingsPath()) as { subagents?: unknown }).subagents,
				undefined,
			);
		});
