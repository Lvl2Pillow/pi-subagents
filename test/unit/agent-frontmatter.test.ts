...
import {
	parseChain,
	serializeChain,
} from "../../src/agents/chain-serializer.ts";
import {
	discoverAgents,
	discoverAgentsAll,
	type AgentConfig,
...
function withTempHome<T>(fn: (home: string) => T): T {
	const home = fs.mkdtempSync(
		path.join(os.tmpdir(), "pi-subagents-package-home-"),
	);
	tempDirs.push(home);
	...
	try {
		...
		if (oldPiCodingAgentDir === undefined)
			delete process.env.PI_CODING_AGENT_DIR;
		else process.env.PI_CODING_AGENT_DIR = oldPiCodingAgentDir;
		if (oldExtraAgentDirs === undefined)
			delete process.env.PI_SUBAGENT_EXTRA_AGENT_DIRS;
...
body`);
		...
		assert.equal(
			afterIndented.frontmatter.description,
			"normal\n  code\n\nnext",
		);
		assert.equal(
			beforeIndented.frontmatter.description,
			"normal\n\n  code\nnext",
		);
	});
});
...
describe("agent skillPath frontmatter", () => {
	it("parses and serializes comma-separated paths", () =>
		withTempHome(() => {
			const project = fs.mkdtempSync(
				path.join(os.tmpdir(), "pi-subagents-skill-path-agent-"),
			);
...
body`,
			...
			const worker = discoverAgents(project, "both").agents.find(
				(agent) => agent.name === "worker",
			)!;
			assert.deepEqual(worker.skillPath, ["./skills", "../shared-skills"]);
			assert.match(
				serializeAgent(worker),
				/^skillPath: \.\/skills, \.\.\/shared-skills$/m,
			);
		}));
});

describe("agent aliases", () => {
	it("parses and serializes agent aliases", () =>
		withTempHome(() => {
			const project = fs.mkdtempSync(
				path.join(os.tmpdir(), "pi-subagents-alias-agent-"),
			);
			tempDirs.push(project);
			writeAgent(
				path.join(project, ".pi", "agents", "worker.md"),
...
body`,
	...
	it("reports management alias collisions as ambiguous", () =>
		withTempHome(() => {
			const project = fs.mkdtempSync(
				path.join(os.tmpdir(), "pi-subagents-alias-collision-"),
			);
			tempDirs.push(project);
			writeAgent(
				path.join(project, ".pi", "agents", "review-agent.md"),
				`---
name: review-agent
...
body`,
			...
			const disableResult = handleManagementAction(
				"disable",
				{ agent: "developer" },
				ctx,
			);
...
describe("agent simple-scalar list frontmatter", () => {
	it("discovers newline block lists for all list fields and routes MCP tools", () => {
		const dir = fs.mkdtempSync(
			path.join(os.tmpdir(), "pi-subagents-agent-block-list-frontmatter-"),
		);
		tempDirs.push(dir);
		writeAgent(
			path.join(dir, ".pi", "agents", "worker.md"),
			`---
name: worker
description: Worker
tools:
...
`,
		...
		const worker = discoverAgents(dir, "project").agents.find(
			(agent) => agent.name === "worker",
		);
		...
		assert.deepEqual(worker?.skillPath, [
			"./private-skills",
			"../shared-skills",
		]);
		assert.deepEqual(worker?.fallbackModels, [
			"openai/gpt-5-mini",
			"anthropic/claude-sonnet-4",
		]);
		assert.deepEqual(worker?.extensions, [
			"./extension-one.ts",
			"./extension-two.ts",
		]);
	...
	it("preserves MCP-only tools as an explicit empty builtin allowlist", () => {
		const dir = fs.mkdtempSync(
			path.join(os.tmpdir(), "pi-subagents-agent-mcp-only-frontmatter-"),
		);
		tempDirs.push(dir);
		writeAgent(
			path.join(dir, ".pi", "agents", "mcp-only.md"),
			`---
name: mcp-only
...
`,
		...
		const agent = discoverAgents(dir, "project").agents.find(
			(candidate) => candidate.name === "mcp-only",
		);
	...
	it("preserves comma-separated syntax across all list fields", () => {
		const dir = fs.mkdtempSync(
			path.join(os.tmpdir(), "pi-subagents-agent-comma-list-frontmatter-"),
		);
		tempDirs.push(dir);
		writeAgent(
			path.join(dir, ".pi", "agents", "worker.md"),
...
describe("agent frontmatter defaultContext", () => {
	...
	it("parses defaultContext from discovered agent frontmatter", () => {
		const dir = fs.mkdtempSync(
			path.join(os.tmpdir(), "pi-subagents-agent-default-context-"),
		);
		tempDirs.push(dir);
		const agentsDir = path.join(dir, ".pi", "agents");
		fs.mkdirSync(agentsDir, { recursive: true });
...
describe("agent frontmatter launch defaults", () => {
	it("serializes and discovers single-agent launch defaults", () => {
		const dir = fs.mkdtempSync(
			path.join(os.tmpdir(), "pi-subagents-agent-launch-defaults-"),
		);
		tempDirs.push(dir);
		...
		assert.match(
			serialized,
			/^acceptance: \{"level":"none","reason":"lightweight lookup"\}$/m,
		);
		...
		assert.deepEqual(worker?.defaultTurnBudget, {
			maxTurns: 12,
			graceTurns: 2,
		});
		assert.deepEqual(worker?.defaultAcceptance, {
			level: "none",
			reason: "lightweight lookup",
		});
	});

	it("parses scalar acceptance defaults and rejects invalid policies", () => {
		const dir = fs.mkdtempSync(
			path.join(os.tmpdir(), "pi-subagents-agent-acceptance-defaults-"),
		);
		tempDirs.push(dir);
		const filePath = path.join(dir, ".pi", "agents", "worker.md");
		writeAgent(
...
`,
		...
		assert.equal(
			discoverAgents(dir, "project").agents.find(
				(agent) => agent.name === "worker",
			)?.defaultAcceptance,
			"checked",
		);
...
`,
	...
	it("parses, serializes, and validates acceptance roles", () => {
		const dir = fs.mkdtempSync(
			path.join(os.tmpdir(), "pi-subagents-agent-acceptance-role-"),
		);
		tempDirs.push(dir);
...
`,
	...
	it("rejects invalid launch defaults instead of silently ignoring them", () => {
		const dir = fs.mkdtempSync(
			path.join(os.tmpdir(), "pi-subagents-agent-invalid-launch-defaults-"),
		);
		tempDirs.push(dir);
		writeAgent(
			path.join(dir, ".pi", "agents", "worker.md"),
...
describe("chain discovery", () => {
	it("prefers same-scope .chain.json over .chain.md for the same runtime name", () => {
		const dir = fs.mkdtempSync(
			path.join(os.tmpdir(), "pi-subagents-chain-format-precedence-"),
		);
		tempDirs.push(dir);
...
`,
		...
		const chain = result.chains.find(
			(candidate) => candidate.name === "dynamic-review",
		);
...
describe("package-provided agents and chains", () => {
	it("discovers package agents and chains from installed package manifests", () =>
		withTempHome(() => {
			const dir = fs.mkdtempSync(
				path.join(os.tmpdir(), "pi-subagents-package-discovery-"),
			);
			tempDirs.push(dir);
...
`,
			...
			const packagedAgent = all.package.find(
				(agent) => agent.name === "my-workflow.reviewer",
			);
			assert.ok(packagedAgent);
			assert.equal(packagedAgent.source, "package");
			assert.equal(
				packagedAgent.filePath,
				path.join(workflowRoot, "agents", "reviewer.md"),
			);
			assert.equal(
				discoverAgents(dir, "both").agents.find(
					(agent) => agent.name === "my-workflow.reviewer",
				)?.source,
				"package",
			);

			const packagedChain = all.chains.find(
				(chain) => chain.name === "my-workflow.review",
			);
	...
	it("loads packages referenced from Pi settings", () =>
		withTempHome(() => {
			const dir = fs.mkdtempSync(
				path.join(os.tmpdir(), "pi-subagents-settings-package-"),
			);
			tempDirs.push(dir);
...
`,
	...
	it("discovers project package agents when cwd is nested below the project root", () =>
		withTempHome(() => {
			const dir = fs.mkdtempSync(
				path.join(os.tmpdir(), "pi-subagents-nested-package-discovery-"),
			);
			tempDirs.push(dir);
...
`,
	...
	it("keeps nearest project root discovery by default when a nested .pi exists", () =>
		withTempHome(() => {
			const dir = fs.mkdtempSync(
				path.join(os.tmpdir(), "pi-subagents-nearest-root-default-"),
			);
			tempDirs.push(dir);
			const nested = path.join(dir, "packages", "app", "src");
...
`,
			...
			const agent = discoverAgents(nested, "both").agents.find(
				(candidate) => candidate.name === "outer-workflow.planner",
			);
			assert.equal(agent, undefined);
		}));

	it("can resolve project packages and overrides from the git root", () =>
		withTempHome(() => {
			const dir = fs.mkdtempSync(
				path.join(os.tmpdir(), "pi-subagents-git-root-resolution-"),
			);
			tempDirs.push(dir);
			const nested = path.join(dir, "packages", "app", "src");
			const nestedConfigDir = path.join(dir, "packages", "app", ".pi");
...
`,
	...
	it("does not register legacy skill files from broad package agent roots", () =>
		withTempHome(() => {
			const dir = fs.mkdtempSync(
				path.join(os.tmpdir(), "pi-subagents-broad-package-skills-"),
			);
			tempDirs.push(dir);
			const packageRoot = path.join(
				dir,
...
`,
			...
			assert.ok(
				packageAgents.find(
					(agent) =>
						agent.name === "package-agent" &&
						agent.filePath === path.join(packageRoot, "agent.md"),
				),
			);
			assert.ok(
				packageAgents.find(
	...
	it("prunes repo internals and nested project roots from broad package discovery", () =>
		withTempHome(() => {
			const dir = fs.mkdtempSync(
				path.join(os.tmpdir(), "pi-subagents-broad-package-prune-"),
			);
			tempDirs.push(dir);
...
`,
			...
			writeAgent(
				path.join(dir, "node_modules", "bad-agent.md"),
				`---
...
`,
			...
			writeAgent(
				path.join(dir, ".git", "hooks", "bad-agent.md"),
				`---
name: git-agent
description: Should not be discovered
---
...
`,
			...
			writeAgent(
				path.join(dir, "vendor", "submodule", "submodule-agent.md"),
				`---
...
`,
			...
			fs.writeFileSync(
				path.join(dir, "vendor", "submodule", ".git"),
				"gitdir: ../.git/modules/submodule\n",
				"utf-8",
			);
			writeAgent(
				path.join(dir, "packages", "app", "local-agent.md"),
				`---
...
`,
			...
			fs.mkdirSync(path.join(dir, "packages", "app", ".pi"), {
				recursive: true,
			});
			writeAgent(
				path.join(dir, "node_modules", "bad.chain.md"),
				`---
...
`,
			...
			assert.ok(
				all.package.find(
					(agent) =>
						...
						agent.filePath === path.join(dir, "root-agent.md"),
				),
			);
			for (const name of [
				"node-modules-agent",
			...
			assert.ok(
				all.chains.find(
					(chain) =>
						chain.name === "root-chain" &&
						chain.filePath === path.join(dir, "root.chain.md"),
				),
			);
			assert.equal(
				all.chains.some((chain) => chain.name === "node-modules-chain"),
				false,
			);
		}));
	...
	it("keeps package definitions below user and project overrides", () =>
		withTempHome((home) => {
			const dir = fs.mkdtempSync(
				path.join(os.tmpdir(), "pi-subagents-package-precedence-"),
			);
			tempDirs.push(dir);
			const packageRoot = path.join(
				dir,
...
`,
			...
			assert.equal(
				discoverAgents(dir, "user").agents.find(
			...
			const chainByName = new Map(
				discoverAgentsAll(dir).chains.map((chain) => [chain.name, chain]),
			);
			assert.equal(chainByName.get("shared")?.source, "project");
		}));

	it("does not allow management updates to package agents", () =>
		withTempHome(() => {
			const dir = fs.mkdtempSync(
				path.join(os.tmpdir(), "pi-subagents-package-readonly-"),
			);
			tempDirs.push(dir);
			const packageRoot = path.join(
				dir,
...
describe("agent frontmatter completionGuard", () => {
	...
	it("parses completionGuard from discovered agent frontmatter", () => {
		const dir = fs.mkdtempSync(
			path.join(os.tmpdir(), "pi-subagents-agent-completion-guard-"),
		);
		tempDirs.push(dir);
		const agentsDir = path.join(dir, ".pi", "agents");
		fs.mkdirSync(agentsDir, { recursive: true });
...
describe("agent frontmatter maxSubagentDepth", () => {
	...
	it("parses maxSubagentDepth from discovered agent frontmatter", () => {
		const dir = fs.mkdtempSync(
			path.join(os.tmpdir(), "pi-subagents-agent-frontmatter-"),
		);
		tempDirs.push(dir);
		const agentsDir = path.join(dir, ".pi", "agents");
		fs.mkdirSync(agentsDir, { recursive: true });
...
describe("agent frontmatter thinking", () => {
	it("coerces frontmatter false strings to disabled thinking", () => {
		const dir = fs.mkdtempSync(
			path.join(os.tmpdir(), "pi-subagents-agent-thinking-false-"),
		);
		tempDirs.push(dir);
		const agentsDir = path.join(dir, ".pi", "agents");
		fs.mkdirSync(agentsDir, { recursive: true });
...
`,
	...
	it("preserves supported frontmatter thinking strings", () => {
		const dir = fs.mkdtempSync(
			path.join(os.tmpdir(), "pi-subagents-agent-thinking-levels-"),
		);
		tempDirs.push(dir);
		const agentsDir = path.join(dir, ".pi", "agents");
		fs.mkdirSync(agentsDir, { recursive: true });
...
`,
		...
		for (const level of THINKING_LEVELS) {
			const agent = agents.find(
				(candidate) => candidate.name === `thinker-${level}`,
			);
			assert.ok(agent);
...
describe("agent frontmatter fallbackModels", () => {
	...
	it("parses fallbackModels from discovered agent frontmatter", () => {
		const dir = fs.mkdtempSync(
			path.join(os.tmpdir(), "pi-subagents-agent-fallback-frontmatter-"),
		);
		tempDirs.push(dir);
		const agentsDir = path.join(dir, ".pi", "agents");
		fs.mkdirSync(agentsDir, { recursive: true });
...
describe("agent frontmatter prompt inheritance flags", () => {
	...
	it("parses inheritProjectContext and inheritSkills from discovered agent frontmatter", () => {
		const dir = fs.mkdtempSync(
			path.join(
				os.tmpdir(),
				"pi-subagents-agent-prompt-inheritance-frontmatter-",
			),
		);
		tempDirs.push(dir);
		const agentsDir = path.join(dir, ".pi", "agents");
...
describe("agent frontmatter subagentOnlyExtensions", () => {
	...
	it("parses subagentOnlyExtensions from discovered agent frontmatter", () => {
		const dir = fs.mkdtempSync(
			path.join(os.tmpdir(), "pi-subagents-agent-child-ext-"),
		);
		tempDirs.push(dir);
		const agentsDir = path.join(dir, ".pi", "agents");
		fs.mkdirSync(agentsDir, { recursive: true });
...
`,
	...
	it("discovers a read-only planner with repository-only tools", () => {
		const dir = fs.mkdtempSync(
			path.join(os.tmpdir(), "pi-subagents-planner-tools-"),
		);
		tempDirs.push(dir);
		const agentsDir = path.join(dir, ".pi", "agents");
		fs.mkdirSync(agentsDir, { recursive: true });
...
`,
		...
		const planner = discoverAgents(dir, "project").agents.find(
			(agent) => agent.name === "planner",
		);
		assert.ok(planner, "planner should be discovered");
		assert.deepEqual(planner.tools, [
			"read",
			"grep",
			"find",
			"ls",
	...
	it("worker and delegate include the child-facing supervisor tool", () => {
		const dir = fs.mkdtempSync(
			path.join(os.tmpdir(), "pi-subagents-supervisor-tool-"),
		);
		tempDirs.push(dir);
		const agentsDir = path.join(dir, ".pi", "agents");
		fs.mkdirSync(agentsDir, { recursive: true });
...
`,
	...
	it("defaults delegate to append mode with inherited project context", () => {
		const dir = fs.mkdtempSync(
			path.join(
				os.tmpdir(),
				"pi-subagents-agent-delegate-default-prompt-settings-",
			),
		);
		tempDirs.push(dir);
		const agentsDir = path.join(dir, ".pi", "agents");
...
describe("packaged agent and chain discovery", () => {
	it("recursively discovers nested project agents while keeping chain files separate", () => {
		const dir = fs.mkdtempSync(
			path.join(os.tmpdir(), "pi-subagents-recursive-agent-discovery-"),
		);
		tempDirs.push(dir);
		const nestedDir = path.join(dir, ".pi", "agents", "code-analysis", "deep");
		const nestedChainDir = path.join(
			dir,
			".pi",
			"chains",
			"code-analysis",
...
`,
		...
		assert.ok(
			result.project.find(
				(agent) =>
					agent.name === "scout" &&
					agent.filePath === path.join(nestedDir, "scout.md"),
			),
		);
		assert.ok(
			result.chains.find(
		...
		assert.equal(
			result.project.some((agent) =>
				agent.filePath.endsWith("review.chain.md"),
			),
			false,
		);
	});
	...
	it("registers packaged agents by runtime name and serializes local name plus package", () => {
		const dir = fs.mkdtempSync(
			path.join(os.tmpdir(), "pi-subagents-packaged-agent-"),
		);
		tempDirs.push(dir);
		const agentsDir = path.join(dir, ".pi", "agents");
		fs.mkdirSync(agentsDir, { recursive: true });
...
`,
	...
	it("recursively discovers packaged chains by runtime name and preserves package on serialize", () => {
		const dir = fs.mkdtempSync(
			path.join(os.tmpdir(), "pi-subagents-packaged-chain-"),
		);
		tempDirs.push(dir);
...
`;
	...
	it("keeps packaged and un-packaged runtime names distinct while preserving un-packaged precedence", () => {
		const dir = fs.mkdtempSync(
			path.join(os.tmpdir(), "pi-subagents-packaged-collisions-"),
		);
		tempDirs.push(dir);
...
`,
	...
	it("normalizes package frontmatter consistently for agents and chains", () => {
		const dir = fs.mkdtempSync(
			path.join(os.tmpdir(), "pi-subagents-package-normalize-"),
		);
		tempDirs.push(dir);
		const agentsDir = path.join(dir, ".pi", "agents");
		const chainsDir = path.join(dir, ".pi", "chains");
...
`,
		...
		assert.ok(
			result.chains.find((chain) => chain.name === "code-analysis.review-flow"),
		);
	});

	it("skips invalid package frontmatter that cannot be normalized", () => {
		const dir = fs.mkdtempSync(
			path.join(os.tmpdir(), "pi-subagents-invalid-package-"),
		);
		tempDirs.push(dir);
		const agentsDir = path.join(dir, ".pi", "agents");
		const chainsDir = path.join(dir, ".pi", "chains");
...
describe("project agent directory discovery", () => {
	it("discovers project agents from both .agents and .pi/agents", () => {
		const dir = fs.mkdtempSync(
			path.join(os.tmpdir(), "pi-subagents-project-agent-dirs-"),
		);
		tempDirs.push(dir);
...
`,
		...
		assert.ok(
			result.agents.find(
				(agent) =>
					agent.name === "legacy" &&
					agent.filePath === path.join(dir, ".agents", "legacy.md"),
			),
		);
		assert.ok(
			result.agents.find(
	...
	it("does not register legacy project skill files as agents", () => {
		const dir = fs.mkdtempSync(
			path.join(os.tmpdir(), "pi-subagents-project-skills-not-agents-"),
		);
		tempDirs.push(dir);
		writeAgent(
			path.join(dir, ".agents", "legacy.md"),
...
`,
	...
	it("does not register user SKILL.md files as agents", () =>
		withTempHome((home) => {
			const dir = fs.mkdtempSync(
				path.join(os.tmpdir(), "pi-subagents-user-skills-not-agents-"),
			);
			tempDirs.push(dir);
			writeAgent(
...
`,
	...
	it("prefers .pi/agents over .agents on project agent name collisions", () => {
		const dir = fs.mkdtempSync(
			path.join(os.tmpdir(), "pi-subagents-project-agent-collision-"),
		);
		tempDirs.push(dir);
		fs.mkdirSync(path.join(dir, ".agents"), { recursive: true });
		fs.mkdirSync(path.join(dir, ".pi", "agents"), { recursive: true });
...
`,
		...
		const shared = discoverAgents(dir, "project").agents.find(
	...
	it("uses the project root for the canonical project agent dir even when only .agents exists", () => {
		const dir = fs.mkdtempSync(
			path.join(os.tmpdir(), "pi-subagents-project-agent-root-"),
		);
		tempDirs.push(dir);
	...
	it("discovers project chains from .pi/chains", () => {
		const dir = fs.mkdtempSync(
			path.join(os.tmpdir(), "pi-subagents-project-chain-dirs-"),
		);
		tempDirs.push(dir);
		fs.mkdirSync(path.join(dir, ".pi", "agents"), { recursive: true });
...
`,
	...
	it("prefers project .pi/chains over user chains on name collisions", () => {
		const dir = fs.mkdtempSync(
			path.join(os.tmpdir(), "pi-subagents-project-chain-collision-"),
		);
		const home = fs.mkdtempSync(
			path.join(os.tmpdir(), "pi-subagents-user-chain-home-"),
		);
...
`,
			...
			const savedChainLookup = new Map(
				sharedChains.map((chain) => [chain.name, chain]),
			);
