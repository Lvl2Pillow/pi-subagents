...
try {
	...
	const schemas =
		(await import("../../src/extension/schemas.ts")) as unknown as {
	...
	chainItemProperties = schemas.ChainItem.properties as
		Record<string, unknown> | undefined;
	dynamicTemplateProperties = schemas.DynamicParallelTemplateSchema
...
describe(
	...
	() => {
		it("is a no-op when chain is absent or not an array", () => {
			...
			assert.doesNotThrow(() =>
				validateChainInput({ agent: "worker", task: "do X" }),
			);
		...
		it("accepts valid chain steps without throwing", () => {
			assert.doesNotThrow(() =>
				validateChainInput({ chain: [{ agent: "worker", task: "do X" }] }),
			);
			assert.doesNotThrow(() =>
				validateChainInput({
					chain: [
			...
			try {
				...
				for (const key of allowed) {
					assert.match(
						error.message,
						new RegExp(`\\b${key}\\b`),
						`allowed key ${key} should appear in message: ${error.message}`,
					);
				}
			}
		});
			...
			try {
				...
				for (const key of allowed) {
					assert.match(
						error.message,
						new RegExp(`\\b${key}\\b`),
						`allowed key ${key} should appear: ${error.message}`,
					);
				}
			}
		});
			...
			expectInvalid(
				{
					chain: [
						{
							expand: {
								from: { output: "x", path: "/items", bogus: true },
								maxItems: 4,
							},
							parallel: { agent: "worker" },
							collect: { as: "reviews" },
...
describe(
	"registered subagent tool prepareArguments",
	{ skip: !schemasAvailable ? "typebox not available" : undefined },
	() => {
		function runPrepare(
			args: unknown,
		...
		it("is a no-op for management actions and single/parallel calls", () => {
			...
			assert.equal(
				runPrepare({ tasks: [{ agent: "worker", task: "do X" }] }).ok,
				true,
			);
		});
	},
