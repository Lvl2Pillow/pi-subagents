...
// This file loads the extension (for the subagent tool), so point its state
// root at a throwaway dir — startup logging / persistent-chat wiring must
// never touch the real extension root.
const __filename = fileURLToPath(import.meta.url);
const TEST_ROOT = path.resolve(path.dirname(__filename), "..", "..");
const STATE_DIR = path.join(TEST_ROOT, ".tmp-e2e-state-real-session-subagent");
...
const BOGUS_EXTRA_DIRS = path.join(
	os.tmpdir(),
	"nonexistent-pi-subagents-e2e-extra-dirs",
);
...
describe(
	...
	() => {
		...
		it("loads requested extension tools in direct and chain children and diagnoses missing providers", async () => {
			const { runRealSubagentSession, subagentCall, subagentToolResults } =
	...
	pi.registerTool({
		...
		async execute() { return { content: [{ type: "text", text: "fixture result" }] }; },
			...
			run = await runRealSubagentSession({
				...
				respond(context) {
					const resultCount = (
						context.messages as Array<{ role?: string; toolName?: string }>
					).filter(
						(message) =>
						...
						return subagentCall(
							{
								...
								task: "Report active tools.",
								context: "fresh",
								agentScope: "project",
							},
							"call-direct-extension",
						);
					}
					if (resultCount === 1) {
						return subagentCall(
							{
								chain: [
									{ agent: "extension-worker", task: "Report active tools." },
								],
								async: false,
								clarify: false,
								agentScope: "project",
					...
					if (resultCount === 3) {
						return subagentCall(
							{
								agent: "missing-extension-worker",
								task: "Report active tools.",
								context: "fresh",
								agentScope: "project",
							},
			...
			const toolMessages = run.parentSession.messages.filter(
				(message) =>
					message.role === "toolResult" &&
					(message as { toolName?: string }).toolName === "subagent",
			);
			const chainDetails = JSON.stringify(
				(toolMessages[1] as { details?: unknown } | undefined)?.details,
			);
			const structuredDetails = JSON.stringify(
				(toolMessages[2] as { details?: unknown } | undefined)?.details,
			);
			...
			assert.match(
				results[3] ?? "",
				/requested unavailable child tools: missing_search/,
			);
