...
describe("real Pi-session subagent E2E", { skip: !available ? "pi runtime packages not available" : undefined }, () => {
	...
	it("loads requested extension tools in workflow children and diagnoses missing providers", async () => {
	...
	pi.registerTool({
		...
		run = await runRealSubagentSession({
			...
			respond(context) {
				...
				if (resultCount === 0) {
					return subagentCall({
						workflowScript: `return runs.run("main", { agent: "extension-worker", task: "Report active tools.", context: "fresh" });`,
						async: false,
						agentScope: "project",
				...
				if (resultCount === 2) {
					return subagentCall({
						workflowScript: `return runs.run("main", { agent: "missing-extension-worker", task: "Report active tools.", context: "fresh" });`,
						async: false,
						agentScope: "project",
	...
	pi.on("before_agent_start", async () => {
		...
		run = await runRealSubagentSession({
			...
			respond(context) {
				...
				return subagentCall({
					workflowScript: `return runs.run("main", { agent: "async-extension-worker", task: "Report active tools.", context: "fresh" });`,
					async: false,
					agentScope: "project",
		...
		try {
			run = await runRealSubagentSession({
				...
				respond: routeParentThroughSubagent({
					...
					subagentArgs: {
						workflowScript: `return runs.run("main", { agent: "worker", task: "Return the marker from the faux child provider.", context: "fresh" });`,
						async: false,
						agentScope: "project",
					},
				}),
