...
async function main() {
	...
	const responseText =
		process.env.PI_SUBAGENTS_E2E_CHILD_TEXT ?? "CHILD_REAL_SESSION_OK";
	const reportChildTools =
		process.env.PI_SUBAGENTS_E2E_REPORT_CHILD_TOOLS === "1";
			...
			process.env.PI_SUBAGENT_STRUCTURED_OUTPUT_CAPTURE
				? fauxAssistantMessage(
						fauxToolCall("structured_output", {
							value: { marker: "STRUCTURED_OUTPUT_OK" },
						}),
						{ stopReason: "toolUse" },
					)
				: fauxAssistantMessage(
						fauxText(
							reportChildTools
								? `ACTIVE_TOOLS:${session?.getActiveToolNames().sort().join(",") ?? ""}`
								: responseText,
						),
						{ stopReason: "stop" },
					),
