...
function makeSessionManagerRecorder(options: SessionStubOptions = {}) {
	const manager: SessionManagerStub = {
		...
		getLeafId: () =>
			options.leafId === undefined ? "leaf-current" : options.leafId,
		openSession: () => ({
...
describe(
	...
	() => {
		...
		function readCallArgsForTask(taskText: string): string[] {
			...
			assert.ok(
				args,
				`expected a recorded mock pi call for task '${taskText}'`,
			);
			return args;
		}
		...
		function writeProjectOverride(
			...
			fs.writeFileSync(
				settingsPath,
				JSON.stringify(
					{ subagents: { agentOverrides: { [agentName]: { model } } } },
					null,
					2,
				),
				"utf-8",
		...
		it("uses agent defaultContext fork when launch context is omitted", async () => {
			const parentSessionFile = path.join(tempDir, "parent.jsonl");
			const { manager, openedPaths, branchedLeafIds } =
				makeForkingSessionManagerRecorder({
			...
			assert.deepEqual(readSessionArgsFromCalls(), [
				path.join(tempDir, "fork-1.jsonl"),
			]);
		});
		...
		it("sanitizes inherited signed thinking and forces child thinking off", async () => {
			...
			assert.deepEqual(entries[2].message.content, [
				{ type: "text", text: "answer" },
			]);
		...
		it("forces every foreground fallback attempt off after sanitizing inherited signed thinking", async () => {
			...
			assert.equal(result.isError, undefined);
			const modelArgs = readAllCallArgs().map(
		...
		it("keeps inherited model-scope violations as warnings in fork mode", async () => {
			const childSessionFile = path.join(
				tempDir,
				"fork-model-scope-warning.jsonl",
			);
		...
		it("warns when empty inheritance falls back to an out-of-scope agent model", async () => {
			const childSessionFile = path.join(
				tempDir,
				"fork-empty-model-scope-warning.jsonl",
			);
			const manager = makeSignedThinkingSessionManager(childSessionFile);
			const executor = makeExecutorWithDiscoverAgents(() => ({
				agents: [
			...
			try {
				...
				assert.equal(
					warnings.every((warning) =>
						warning.includes("outside the configured subagent model scope"),
					),
					true,
				);
		...
		it("notes the forced thinking downgrade in the result for Anthropic forked children", async () => {
			...
			const text = result.content
				...
				.join("\n");
			assert.equal(
				text.includes("fork context forced thinking off for worker (child 0)"),
				true,
			);
		});
		...
		it("classifies empty parallel model overrides as parent inheritance", async () => {
			const childSessionFile = path.join(
				tempDir,
				"fork-empty-parallel-model.jsonl",
			);
			const manager = makeSignedThinkingSessionManager(childSessionFile);
			const executor = makeExecutorWithDiscoverAgents(() => ({
				agents: [
		...
		it("classifies the agent model when empty inheritance has no parent model", async () => {
			const childSessionFile = path.join(
				tempDir,
				"fork-empty-model-without-parent.jsonl",
			);
			const manager = makeSignedThinkingSessionManager(childSessionFile);
			const executor = makeExecutorWithDiscoverAgents(() => ({
				agents: [
		...
		it("uses each agent defaultContext for top-level parallel when launch context is omitted", async () => {
			...
			assert.deepEqual(
				result.details?.results?.map((entry) => entry.context),
				["fork", "fresh"],
			);
			assert.deepEqual(openedPaths, [parentSessionFile]);
			assert.deepEqual(branchedLeafIds, ["leaf-current"]);
			...
			assert.match(
				workerArgs.at(-1) ?? "",
				/delegated subagent running from a fork/,
			);
			assert.doesNotMatch(
				freshArgs.at(-1) ?? "",
				/delegated subagent running from a fork/,
			...
			assert.equal(
				readSessionArg(workerArgs),
				path.join(tempDir, "fork-1.jsonl"),
			);
			assert.notEqual(
				readSessionArg(freshArgs),
				path.join(tempDir, "fork-1.jsonl"),
			);
		});
		...
		it("uses each agent defaultContext for chain runs when launch context is omitted", async () => {
			...
			const forkSessionArgs = readSessionArgsFromCalls().filter((sessionFile) =>
				path.basename(sessionFile).startsWith("fork-"),
			);
		...
		it("preflights static default-fork chain steps even when the chain also has dynamic fanout", async () => {
			const parentSessionFile = path.join(
				tempDir,
				"parent-dynamic-chain-fail.jsonl",
			);
			fs.writeFileSync(
				parentSessionFile,
				'{"type":"session","version":1,"id":"parent","timestamp":"2026-04-16T00:00:00.000Z","cwd":"/tmp"}\n',
		...
		it("keeps later foreground forked chain steps aligned after short dynamic fanout", async () => {
			...
			assert.equal(
				readSessionArg(readCallArgsForTask("inspect one")),
				path.join(tempDir, "fork-1.jsonl"),
			);
			assert.equal(
				readSessionArg(readCallArgsForTask("final")),
		...
		it("uses top-level parallel config overrides for maxTasks and concurrency", async () => {
			...
			const maxTasksExecutor = makeExecutorWithConfig({
				parallel: { maxTasks: 9 },
			});
					...
					mockPi.onCall({
						steps: [
							...
							{ delay: 250 },
							{
								jsonl: [
									events.toolEnd("bash"),
									events.assistantMessage(`done-${i}`),
								],
							},
						],
					});
		...
		it("detaches parallel child runs cleanly on supervisor handoff", async () => {
				...
				agents: [
					{
						name: "echo",
						description: "Echo",
						systemPrompt: "Native supervisor orchestration channel:",
					},
					{
						name: "second",
						description: "Second",
			...
			const result = await executor.execute(
				...
				(update: ProgressUpdate) => {
					...
					if (
						...
						return;
					detachEmitted = true;
					executor.eventsApi.emit(SUBAGENT_DETACH_REQUEST_EVENT, {
			...
			assert.match(
				result.content[0]?.text ?? "",
				/Parallel run detached for intercom coordination/,
			);
			assert.equal(detachEmitted, true);
			assert.equal(
				result.details?.results?.some(
					(entry) => entry.detached === true && entry.exitCode === -2,
				),
				true,
			);
		});
		...
		it(
			"runs top-level parallel async requests in the background",
			{ skip: !asyncAvailable ? "jiti not available" : undefined },
			async () => {
				const executor = makeExecutor();
		...
		it(
			"keeps inherited model-scope warnings non-fatal in async parallel conversion",
			{ skip: !asyncAvailable ? "jiti not available" : undefined },
			async () => {
				const executor = makeExecutorWithDiscoverAgents(() => ({
					agents: [
		...
		it(
			"keeps inherited model-scope warnings non-fatal after Clarify switches parallel work to background",
			{ skip: !asyncAvailable ? "jiti not available" : undefined },
			async () => {
				const executor = makeExecutorWithDiscoverAgents(() => ({
					agents: [
		...
		it(
			"keeps raw goals at fork-wrapped async executor boundaries",
			{ skip: !asyncAvailable ? "jiti not available" : undefined },
			async () => {
				...
				executor.eventsApi.on(SUBAGENT_ASYNC_STARTED_EVENT, (event: unknown) =>
					started.push(event as { id?: string; goal?: string }),
				);
				const forkManager = () =>
					makeForkingSessionManagerRecorder({
				...
				for (const testCase of [
					...
					},
					{
						...
						params: {
							tasks: [
								{ agent: "echo", task: rawParallelGoal },
								{ agent: "second", task: "Second child" },
							],
							async: true,
							clarify: false,
						},
					},
					{
						...
						params: {
							...
							chain: [
								{
									parallel: [
										{ agent: "echo", task: rawParallelChainGoal },
										{ agent: "second", task: "Second chain child" },
									],
								},
							],
							async: true,
					...
					},
					{
						...
						params: {
							task: paddedWorkflowGoal,
							chain: [{ agent: "echo", task: "Forked child" }],
							async: true,
							clarify: false,
						},
					},
					{
						...
						params: {
							chain: [{ agent: "second", task: literalPreambleGoal }],
							async: true,
							clarify: false,
						},
					},
					...
					assert.ok(
						result.details?.asyncId,
						`${testCase.name}: expected an async id`,
					);
					const event = started.find(
						(entry) => entry.id === result.details?.asyncId,
					);
					assert.ok(
						event,
						`${testCase.name}: missing async-started event for ${result.details?.asyncId}`,
					);
		...
		it(
			"uses edited raw goals when clarification switches each mode to background",
			{ skip: !asyncAvailable ? "jiti not available" : undefined },
			async () => {
				const executor = makeExecutorWithDiscoverAgents(() => ({
					agents: [
				...
				for (const testCase of [
					...
					},
					{
						...
						params: {
							tasks: [
								{ agent: "echo", task: "Original parallel" },
								{ agent: "second", task: "Second child" },
							],
							async: true,
							clarify: true,
					...
					},
					{
						...
						params: {
							chain: [{ agent: "echo", task: "Original chain" }],
							async: true,
							clarify: true,
						},
					...
					const forkManager = makeForkingSessionManagerRecorder({
						sessionFile: path.join(
							tempDir,
							`parent-clarify-${testCase.name}.jsonl`,
						),
		...
		it(
			"runs async chain requests in the background when clarify is omitted",
			{ skip: !asyncAvailable ? "jiti not available" : undefined },
			async () => {
				const executor = makeExecutor();
				...
				assert.ok(
					result.details?.asyncId,
					"expected an asyncId for background chain runs",
				);
		...
		it("creates isolated forked sessions per chain step (including counted parallel steps)", async () => {
			...
			assert.deepEqual(
				openedPaths,
				Array(6).fill(path.join(tempDir, "parent-chain.jsonl")),
			);
			assert.deepEqual(branchedLeafIds, Array(6).fill("leaf-chain"));
			const sessionArgs = readSessionArgsFromCalls().filter(
				(sessionFile) =>
					path.dirname(sessionFile) === tempDir &&
					path.basename(sessionFile).startsWith("fork-"),
			);
			assert.equal(
				sessionArgs.length,
				6,
				"1 sequential + 4 parallel + 1 sequential",
			);
		...
		it("uses request cwd for management actions", async () => {
			...
			assert.equal(
				fs.existsSync(
					path.join(worktreeDir, ".pi", "agents", "local-helper.md"),
				),
				true,
			);
			assert.equal(
				fs.existsSync(path.join(tempDir, ".pi", "agents", "local-helper.md")),
				false,
			);
		});
		...
		it("resolves parallel task cwd values relative to the request cwd", async () => {
			const worktreeDir = path.join(tempDir, "worktree");
			writePackageSkill(
				path.join(worktreeDir, "packages", "app"),
				"parallel-step-skill",
			);
			const executor = makeExecutorWithDiscoverAgents(() => ({
				agents: [
			...
			assert.equal(result.isError, undefined);
			assert.deepEqual(result.details?.results?.[0]?.skills, [
				"parallel-step-skill",
			]);
		});
