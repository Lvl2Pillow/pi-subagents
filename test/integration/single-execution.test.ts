...
async function withIsolatedWatchdogSettings<T>(
	projectDir: string,
	run: () => Promise<T>,
): Promise<T> {
...
interface ExecutorToolResult {
	...
	details?: {
		...
		results?: Array<{
			...
			acceptance?: {
				status?: string;
				effectiveAcceptance?: { reason?: string };
			};
		}>;
	};
}
...
const execution = await tryImport<ExecutionModule>(
	"./src/runs/foreground/execution.ts",
);
const utils = await tryImport<UtilsModule>("./src/shared/utils.ts");
const executorMod = await tryImport<ExecutorModule>(
...
describe(
	...
	() => {
		...
		it(
			"treats action='single' with execution fields as single execution",
		...
		it(
			"admits a zero run-level tool budget only for marked v2 delegated execution",
			{ skip: !createSubagentExecutor ? "executor not importable" : undefined },
			async () => {
				...
				assert.match(
					v1Delegated.content[0]?.text ?? "",
					/toolBudget\.hard must be an integer >= 1/,
				);

				mockPi.onCall({
					echoEnv: [TOOL_BUDGET_ENV, TOOL_BUDGET_ZERO_AUTH_ENV],
				});
				...
				const env = JSON.parse(v2Delegated.content[0]?.text ?? "{}") as Record<
					string,
					string
				>;
				assert.deepEqual(
					JSON.parse(env[TOOL_BUDGET_ENV] ?? "null"),
					zeroBudget,
				);
		...
		it(
			"keeps delegated agent and config tool budgets at a minimum of one",
			{ skip: !createSubagentExecutor ? "executor not importable" : undefined },
			async () => {
				const ctx = makeMinimalCtx(tempDir);
				const cases = [
				...
				for (const [index, executor] of cases.entries()) {
					...
					assert.match(
						...
						/(?:agent\.|config\.)?toolBudget\.hard must be an integer >= 1/,
					);
				}
	...
	it("routes workflow script children through ordinary foreground execution", { skip: !createSubagentExecutor ? "executor not importable" : undefined }, async () => {
		...
		it(
			...
			async () => {
				...
				assert.match(
					result.content[0]?.text ?? "",
					/acceptance level "none" requires a reason/,
				);
				assert.equal(mockPi.callCount(), 0);
			},
		);
	...
	it("applies a workflow usage budget across scripted child launches", { skip: !createSubagentExecutor ? "executor not importable" : undefined }, async () => {
		...
		it(
			"routes watchdog.configure through the management action path",
			{ skip: !createSubagentExecutor ? "executor not importable" : undefined },
			async () => {
				const gpt = {
				...
				assert.match(
					result.content[0]?.text ?? "",
					/session model configured: anthropic\/claude-opus-4-8:high/,
				);
				assert.equal(
					watchdog.getSnapshot(tempDir).config.main.model,
		...
		it("allows concurrent delegated foreground execution calls", async () => {
			...
			const [first, second] = await Promise.all([
				executor.executeDelegated(
					"first",
					{ agent: "echo", task: "First delegated call" },
					new AbortController().signal,
					undefined,
					ctx,
				),
				executor.executeDelegated(
					"second",
					{ agent: "second", task: "Second delegated call" },
					new AbortController().signal,
					undefined,
					ctx,
				),
		...
		it("routes registered strict v1 delegation through the concurrent executor", async () => {
			mockPi.onCall({
				steps: [
					...
					{
						jsonl: [events.toolEnd("read"), events.toolResult("read", "{}")],
						delay: 20,
					},
					{
			...
			const runtimeHandlers = new Map<
				string,
				Array<(event: unknown, ctx: ReturnType<typeof makeMinimalCtx>) => void>
			>();
			const fakePi = new Proxy(
				{
					...
					on(
						...
						return () =>
							runtimeHandlers.set(
								event,
								(runtimeHandlers.get(event) ?? []).filter(
									(entry) => entry !== handler,
								),
							);
					},
			...
			const ctx = {
				...
				sessionManager: {
					...
					getSessionFile: () =>
						path.join(tempDir, "registered-delegation-session.jsonl"),
					getEntries: () => [],
				},
			};
			...
			extensionEvents.on(SUBAGENT_DELEGATION_UPDATE_EVENT, (payload) =>
				updates.push(payload as SubagentDelegationUpdate),
			);
			...
			try {
				...
				assert.equal(
					mockPi.callCount(),
					2,
					"registered strict v1 requests should bypass the ordinary one-call guard",
				);
				...
				assert.ok(
					responses.every((response) => response.status === "completed"),
				);
				...
				const toolResponse = responses.find(
					(response) => response.output === "registered first delegated call",
				);
				assert.ok(toolResponse);
				assert.equal(
					updates.some(
						(update) =>
							update.requestId === toolResponse.requestId &&
							update.currentTool === "read",
					),
					true,
				);
			} finally {
		...
		it("routes registered strict v2 text delegation through the concurrent executor", async () => {
			...
			const ctx = {
				...
				modelRegistry: {
					getAvailable: () => [
						{ provider: "mock", id: "test-model", reasoning: true },
					],
				},
				sessionManager: {
					getSessionId: () => "registered-delegation-v2-session",
					getSessionFile: () =>
						path.join(tempDir, "registered-delegation-v2-session.jsonl"),
					getEntries: () => [],
				},
			};
			...
			try {
				...
				assert.equal(
					mockPi.callCount(),
					2,
					`different V2 logical nodes should use the concurrent delegated execution path: ${JSON.stringify(responses)}`,
				);
				assert.deepEqual(
					started
						.map(({ requestId, ownerRunId, nodeId }) => ({
							requestId,
							ownerRunId,
							nodeId,
						}))
				...
				const terminalResponses = responses.filter(
					(response) => response.status !== "invalid_request",
				);
				...
				const literalResponse = terminalResponses.find(
					(response) =>
						response.result?.kind === "text" &&
						response.result.text === literalJsonText,
				);
				assert.ok(literalResponse);
				assert.deepEqual(literalResponse.result, {
					kind: "text",
					text: literalJsonText,
				});
				assert.deepEqual(
					literalResponse.usage && {
				...
				const plainResponse = terminalResponses.find(
					(response) =>
						response.result?.kind === "text" &&
						response.result.text === "registered v2 second node",
				);
		...
		it(
			"allows concurrent async launches in one turn",
			...
			async () => {
				...
				const [first, second] = await Promise.all([
					executor.execute(
						"first",
						{ agent: "echo", task: "First", async: true },
						new AbortController().signal,
						undefined,
						ctx,
					),
					executor.execute(
						"second",
						{ agent: "second", task: "Second", async: true },
						new AbortController().signal,
						undefined,
						ctx,
					),
				...
				assert.equal(
					mockPi.callCount(),
					2,
					"both detached mock children should start before test cleanup",
				);
			},
		);
		...
		it("does not impose a cumulative spawn cap by default", async () => {
			...
			assert.equal(
				spawnState.count,
				40,
				"unlimited sessions should bypass cumulative accounting",
			);
		});
		...
		it("preflights static chains before creating run artifacts", async () => {
			...
			assert.match(
				result.content[0]?.text ?? "",
				/0\/1 used, 2 requested\).*1 remaining/,
			);
		...
		it("applies bounded root-interactive spawn-budget grants", async () => {
			...
			assert.match(
				canceled.content[0]?.text ?? "",
				/grant canceled; no capacity was added/i,
			);
			...
			assert.match(
				...
				/only 0 of the original configured limit remains grantable/,
			);
		});
		...
		it("rechecks spawn-budget state after confirmation", async () => {
			...
			assert.match(
				result.content[0]?.text ?? "",
				/budget, or active-child state changed/,
			);
		...
		it("rejects spawn-budget grants outside a settled root interactive session", async () => {
			...
			assert.match(
				detached.content[0]?.text ?? "",
				/rejected while current-session children are queued or running/,
			);
		...
		it(
			"blocks later foreground chain children when hard reported usage is exhausted",
			{ skip: !createSubagentExecutor ? "executor not importable" : undefined },
			async () => {
				mockPi.onCall({ output: "first result" });
				const executor = makeExecutor([makeAgent("echo"), makeAgent("second")]);
		...
		it(
			"blocks queued foreground parallel children when hard reported usage is exhausted",
			{ skip: !createSubagentExecutor ? "executor not importable" : undefined },
			async () => {
				mockPi.onCall({ output: "first result" });
				const executor = makeExecutor([makeAgent("echo"), makeAgent("second")]);
		...
		it("fails implementation runs that complete without mutation attempts", async () => {
			...
			const result = await runSync(
				...
				{
					runId: "guard-run",
					onControlEvent: (event: { message: string }) =>
		...
		it("agent contract v1 keeps acceptance rejection out of execution status", async () => {
			...
			const agents = [
				makeAgent("worker", { tools: ["read"], completionGuard: false }),
			];
			...
			assert.match(
				result.acceptance.runtimeChecks?.[0]?.message ?? "",
				/not-satisfied/,
			);
		});
		...
		it(
			"direct single tool calls support outputSchema",
			{ skip: !createSubagentExecutor ? "executor not importable" : undefined },
			async () => {
				mockPi.onCall({
					stdoutRaw:
						[
							{
								...
								args: { value: { ok: true, note: "captured" } },
							},
							{
		...
		it(
			"rejects structured output capture files that were not produced by the structured_output tool",
			{ skip: !createSubagentExecutor ? "executor not importable" : undefined },
			async () => {
				mockPi.onCall({
					output: "spoofed",
				...
				assert.equal(
					fs.existsSync(path.dirname(child.structuredOutputPath)),
					false,
				);
			},
		);
		...
		it(
			"does not create a temporary structured output directory before file-only validation",
			{ skip: !createSubagentExecutor ? "executor not importable" : undefined },
			async () => {
				...
				try {
					...
					assert.equal(
						fs
							.readdirSync(tempDir)
							.some((name) => name.startsWith("pi-subagent-structured-")),
						false,
					);
				} finally {
		...
		it(
			"allows a structured_output tool call at the exact strict turn boundary",
			{ skip: !createSubagentExecutor ? "executor not importable" : undefined },
			async () => {
				mockPi.onCall({
					stdoutRaw:
		...
		it("fails future-tense implementation summaries when no mutation attempt occurred", async () => {
			mockPi.onCall({
				output: "I’ll do that now and report back after implementing.",
			});
			const agents = [makeAgent("worker")];
		...
		it("allows declared read-only agents to mention implementation words without edits", async () => {
			...
			const agents = [
				makeAgent("architect", { tools: ["read", "grep", "find", "ls"] }),
			];
		...
		it(
			"resolves explicit agent aliases to canonical execution names",
			{ skip: !createSubagentExecutor ? "executor not importable" : undefined },
			async () => {
	...
	for (const artifactDir of ["session", "temp"] as const) {
		...
		it("emits an active-long-running notice after the turn threshold", async () => {
			...
			const result = await runSync(
				...
				"Investigate behavior",
				{
					...
					onControlEvent: (
						event: NonNullable<RunSyncResult["controlEvents"]>[number],
					) => controlEvents.push(event),
				},
			);
		...
		it("escalates repeated mutating tool failures to needs attention", async () => {
			mockPi.onCall({
				jsonl: [
					events.toolStart("edit", {
						path: "src/runs/background/async-status.ts",
					}),
					events.toolEnd("edit"),
					events.toolResult(
						"edit",
						"No exact match found for async-status.ts",
						true,
					),
					events.toolStart("edit", {
						path: "src/runs/background/async-status.ts",
			...
			assert.equal(result.exitCode, 0);
			const failureEvent = controlEvents.find(
				(event) => event.reason === "tool_failures",
			);
			assert.equal(failureEvent?.type, "needs_attention");
			assert.equal(
				failureEvent?.currentPath,
			...
			"does not retry non-SIGKILL signaled child exits",
			{
				skip:
					process.platform === "win32"
						? "POSIX child signal reporting is unavailable on Windows"
						: false,
			},
			async () => {
		...
		it(
			"prefers signal termination errors over stderr tails",
			{
				skip:
					process.platform === "win32"
						? "POSIX child signal reporting is unavailable on Windows"
			...
			async () => {
				mockPi.onCall({
					stderr: "INFO benign startup line\n",
					signal: "SIGTERM",
				});
				const agents = [makeAgent("worker", { model: "openai/gpt-5-mini" })];
		...
		it("does not retry a child exit with raw stdout diagnostics", async () => {
			mockPi.onCall({
				stdoutRaw: "configuration failed before protocol startup\n",
				exitCode: 1,
			});
			mockPi.onCall({ output: "must not run" });
			const agents = [makeAgent("worker", { model: "openai/gpt-5-mini" })];
			...
			assert.match(
				result.error ?? "",
				/configuration failed before protocol startup/,
			);
			assert.equal(result.modelAttempts?.length, 1);
			assert.equal(mockPi.callCount(), 1);
		});
		...
		it("reports an actionable error after startup retries are exhausted", async () => {
			...
			assert.match(
				result.error ?? "",
				/failed to start after 4 attempts.*concurrent Pi startup race/i,
			);
		...
		it("uses agent model config", async () => {
			...
			const agents = [
				makeAgent("echo", { model: "anthropic/claude-sonnet-4" }),
			];
		...
		it("parses split UTF-8 JSON and a final unterminated protocol line", async () => {
			const line = Buffer.from(
				JSON.stringify(events.assistantMessage("你好 from fragmented JSON")),
			);
			...
			const result = await runSync(
				...
				"echo",
				"Read fragmented output",
				{ acceptance: false },
			);
			assert.equal(result.exitCode, 0);
			assert.equal(
				getFinalOutput(result.messages),
				"你好 from fragmented JSON",
			);
		});

		it("fails with protocol_output_limit when a child emits an oversized stdout line", async () => {
			mockPi.onCall({
				stdoutRaw: "x".repeat(MAX_CHILD_PENDING_LINE_BYTES + 1),
			});
			const result = await runSync(
				tempDir,
				makeAgentConfigs(["echo"]),
		...
		it("keeps only a bounded UTF-8 stderr tail", async () => {
			mockPi.onCall({
				output: "failed",
				stderr: `${"x".repeat(MAX_CHILD_STDERR_BYTES + 1024)}终`,
				exitCode: 1,
			});
			const result = await runSync(
				tempDir,
			...
			assert.equal(result.exitCode, 1);
			assert.ok(
				Buffer.byteLength(result.error ?? "") <= MAX_CHILD_STDERR_BYTES,
			);
		...
		it("cancels final drain while agent_end reports a retry and waits for agent_settled", async () => {
			mockPi.onCall({
				steps: [
					{
						jsonl: [
							events.assistantMessage("retrying response"),
					...
					},
					{
						delay: 1400,
						jsonl: [
							events.assistantMessage("settled response"),
			...
			assert.ok(
				...
				"foreground runner must not terminate during the retry delay",
			);
		});

		it("treats agent_settled as a clean terminal watermark", async () => {
			const nonTerminalMessage = events.assistantMessage(
				"settled without a terminal assistant stop",
			) as { message: { stopReason: string } };
			nonTerminalMessage.message.stopReason = "length";
			mockPi.onCall({
				jsonl: [nonTerminalMessage, { type: "agent_settled" }],
			...
			assert.ok(
				...
				"agent_settled should trigger bounded child cleanup",
			);
		});
				...
				jsonl: [
					{
						...
						message: {
							...
							model: "openai/gpt-5-mini",
							errorMessage:
								"429 you have reached your weekly usage limit / quota exceeded",
							usage: {
								input: 10,
								output: 5,
								cacheRead: 0,
		...
		it("baselines output files per fallback attempt", async () => {
			...
			setTimeout(() => {
				fs.writeFileSync(
					outputPath,
					"stale partial output from failed primary",
					"utf-8",
				);
			}, 20);
		...
		it("does not retry on ordinary task/tool failures", async () => {
			mockPi.onCall({
				jsonl: [
					events.toolResult("bash", "process exited with code 127", true),
				],
				exitCode: 0,
			});
			const agents = [
		...
		it("tracks live activity updates and exposes artifact paths while running", async () => {
			...
			mockPi.onCall({
				steps: [
					...
					},
					{
						jsonl: [
							events.toolEnd("read"),
							events.toolResult("read", '{"name":"pkg"}'),
						],
						delay: 20,
					},
			...
			assert.ok(
				updates.length > 0,
				"expected at least one live progress update",
			);
			assert.equal(
				updates.some(
					(update) =>
						update.details?.results?.[0]?.artifactPaths?.outputPath.endsWith(
							"_output.md",
						) === true,
				),
				true,
			...
			assert.ok(
				runningToolUpdate,
				"expected a live progress update for the running tool",
			);
			assert.equal(
				runningToolUpdate?.details?.progress?.[0]?.currentTool,
				"read",
			);
			assert.equal(
		...
		it("does not flag a delayed active tool as idle attention", async () => {
			...
			mockPi.onCall({
				steps: [
					...
					{
						delay: 2_000,
						jsonl: [events.toolEnd("bash"), events.toolResult("bash", "done")],
					},
					{ jsonl: [events.assistantMessage("Done")] },
				],
			});
			...
			assert.equal(result.progress.activityState, undefined);
			assert.equal(
				controlEvents.some((event) => event.type === "needs_attention"),
				false,
			);
			assert.equal(
				updates.some((update) =>
					update.details?.progress?.some(
						(progress) => progress.currentTool === "bash",
					),
				),
				true,
			);
		...
		it("injects an agent-file-relative local skill into the foreground child prompt", async () => {
			...
			const prompt = readCall()
				.systemPrompts.map((record) => record.text ?? "")
				.join("\n");
		...
		it("falls back to the runtime cwd when the task cwd lacks a skill", async () => {
			...
			mockPi.onCall({ output: "Done" });
			const agents = [
				makeAgent("echo", { skills: ["runtime-fallback-skill"] }),
			];
		...
		it("writes artifacts when configured", async () => {
			...
			const privateExtension = path.join(
				tempDir,
				"extensions",
			...
			assert.equal(
				result.launchResolvedExtensions?.disableAmbientExtensions,
				true,
			);
			assert.deepEqual(
				metadata.launchResolvedExtensions,
				result.launchResolvedExtensions,
			);
			assert.ok(
				!JSON.stringify(result.launchResolvedExtensions).includes(tempDir),
				"projection should not expose raw extension paths",
			);
		});

		it(
			"routes foreground artifacts to the configured session directory",
			{ skip: !createSubagentExecutor ? "executor not importable" : undefined },
			async () => {
				...
				const result = await executor.execute(
					...
					{
						agent: "echo",
						task: "Write session-scoped artifacts",
						runId: "session-artifacts",
					},
					new AbortController().signal,
					undefined,
					ctx,
				...
				assert.ok(
					result.details?.artifacts?.files[0]?.outputPath.startsWith(
				...
				assert.equal(
					fs.existsSync(path.join(tempDir, ".pi-subagents", "artifacts")),
					false,
				);
			},
		);
		...
		it(
			"routes foreground single relative outputs to the run output artifact directory by default",
			{ skip: !createSubagentExecutor ? "executor not importable" : undefined },
			async () => {
				...
				assert.match(
					taskArg,
					new RegExp(
						`Write your findings to exactly this path: ${escapeRegExp(path.join(tempDir, ".pi-subagents", "artifacts", "outputs"))}.*context\\.md`,
					),
				);
		...
		it(
			"routes foreground single relative outputs to configured singleRunOutputBaseDir",
			{ skip: !createSubagentExecutor ? "executor not importable" : undefined },
			async () => {
				...
				assert.match(
					taskArg,
					new RegExp(
						`Write your findings to exactly this path: ${escapeRegExp(expectedOutputPath)}`,
					),
				);
				assert.equal(
		...
		it(
			...
			async () => {
				...
				assert.match(
					taskArg,
					new RegExp(
						`Write your findings to exactly this path: ${escapeRegExp(overridePath)}`,
					),
				);
		...
		it(
			"persists read-only file-only output without requiring a child write tool",
			{ skip: !createSubagentExecutor ? "executor not importable" : undefined },
			async () => {
				...
				for (const instruction of [taskArg, systemPrompt]) {
					...
					assert.match(
						instruction,
						/runtime will persist it to exactly this path:/,
					);
					assert.match(
						instruction,
		...
		it(
			...
			async () => {
				...
				assert.equal(
					fs.existsSync(path.join(tempDir, "default-report.md")),
					false,
				);
				assert.doesNotMatch(
					readCallArgs().at(-1) ?? "",
		...
		it(
			"rejects explicit reviewed acceptance at every execution nesting level before spawning",
			{ skip: !createSubagentExecutor ? "executor not importable" : undefined },
			async () => {
				const cases = [
						...
						chain: [
							{
								...
								parallel: { agent: "echo", acceptance: { level: "reviewed" } },
								collect: { as: "reviews" },
							},
						],
					},
		...
		it(
			"rejects explicit reviewed acceptance before appending a chain step",
			{ skip: !createSubagentExecutor ? "executor not importable" : undefined },
			async () => {
				const executor = makeExecutor([makeAgent("echo")]);
				const result = await executor.execute(
				...
				assert.match(
					result.content[0]?.text ?? "",
					/Cannot append step:.*achieved status.*acceptance\.review\.required/i,
				);
				assert.equal(mockPi.callCount(), 0);
			},
		);
		...
		it(
			"rejects mismatched foreground timeout aliases before spawning",
			{ skip: !createSubagentExecutor ? "executor not importable" : undefined },
			async () => {
				const executor = makeExecutor();
		...
		it(
			"applies the foreground timeout default without overriding explicit or agent values",
			{ skip: !createSubagentExecutor ? "executor not importable" : undefined },
			async () => {
				...
				assert.equal(
					defaultResult.details?.timeoutMs,
					executorMod?.DEFAULT_FOREGROUND_TIMEOUT_MS,
				);
		...
		it(
			"applies agent frontmatter defaults to single-agent launches",
			{
			...
			async () => {
				...
				assert.deepEqual(result.details?.turnBudget, {
					maxTurns: 4,
					graceTurns: 2,
				});
			},
		...
		it(
			"applies agent acceptance defaults and lets explicit calls override them",
			{ skip: !createSubagentExecutor ? "executor not importable" : undefined },
			async () => {
				mockPi.onCall({ output: "default acceptance disabled" });
				mockPi.onCall({
					stdoutRaw: `${JSON.stringify(events.assistantMessage("explicit checked response without a report"))}\n`,
				});
				const executor = makeExecutor([
					makeAgent("echo", {
						defaultAcceptance: {
							level: "none",
							reason: "lightweight response",
						},
					}),
				]);
				...
				assert.equal(
					defaulted.details?.results?.[0]?.acceptance?.status,
					"not-required",
				);
				assert.equal(
				...
				assert.equal(
					explicit.details?.results?.[0]?.acceptance?.status,
					"rejected",
				);
			},
		);
		...
		it(
			"lets agent frontmatter override the global async default",
			{ skip: !createSubagentExecutor ? "executor not importable" : undefined },
			async () => {
				...
				assert.match(
					result.content[0]?.text ?? "",
					/agent foreground default finished/,
				);
				assert.equal(result.details?.asyncId, undefined);
			},
		);
		...
		it(
			"lets explicit single-agent launch values override frontmatter defaults",
			{ skip: !createSubagentExecutor ? "executor not importable" : undefined },
			async () => {
		...
		it("passes fanout routing env only when builtin subagent is declared", async () => {
			...
			try {
				...
				const fanoutAgents = [
					makeAgent("delegator", { tools: ["read", "subagent"] }),
				];
				const fanout = await runSync(
					tempDir,
					fanoutAgents,
					"delegator",
					"Task",
					{ runId: "fanout-run", index: 2 },
				);
				...
				const nonFanout = await runSync(
					tempDir,
					nonFanoutAgents,
					"worker",
					"Task",
					{ runId: "non-fanout-run" },
				);
		...
		it("fails with actionable diagnostics when a requested extension tool is not loaded", async () => {
			mockPi.onCall({
				output: "Model incorrectly claimed success",
				missingTools: ["fixture_search"],
			});
			const agents = [
				makeAgent("extension-worker", {
			...
			const result = await runSync(
				...
				"extension-worker",
				"Use fixture search",
				{ runId: "missing-extension-tool" },
			);

			assert.equal(result.exitCode, 1);
			assert.match(
				result.error ?? "",
				/requested unavailable child tools: fixture_search/,
			);
		...
		it(
			...
			async () => {
				...
				const extensionArgs = args.filter(
					(_, index) => args[index - 1] === "--extension",
				);
				assert.ok(
				...
				assert.ok(
					extensionArgs.some((arg) =>
						arg.replace(/\\/g, "/").endsWith("custom-tool.ts"),
					),
				);
				assert.ok(
					extensionArgs.some((arg) =>
						arg.replace(/\\/g, "/").endsWith("allowed-ext.ts"),
					),
				);
			},
		);
		...
		it(
			...
			async () => {
				...
				assert.ok(
					extensionArgs.some((arg) =>
						arg.replace(/\\/g, "/").endsWith("child-only-tool.ts"),
					),
				);
			},
		);
		...
		it("ignores child watchdog status when foreground child watchdogs are not configured", async () => {
			await withIsolatedWatchdogSettings(tempDir, async () => {
				mockPi.onCall({
					jsonl: [
						events.assistantMessage("done-without-watchdog-config"),
						childWatchdogStatus("reviewing", 1),
					],
				...
				assert.ok(
					elapsed < 5000,
					`unconfigured watchdog status should not delay final drain, took ${elapsed}ms`,
				);
				assert.equal(result.exitCode, 0);
				assert.equal(result.finalOutput, "done-without-watchdog-config");
				assert.equal(
					(result as RunSyncResult & { watchdog?: unknown }).watchdog,
					undefined,
				);
			});
		});
			...
			await withIsolatedWatchdogSettings(tempDir, async () => {
				...
				mockPi.onCall({
					steps: [
						{
							jsonl: [
								events.assistantMessage("done-before-watchdog"),
								childWatchdogStatus("reviewing", 1),
							],
						},
				...
				assert.ok(
					elapsed >= 1200,
					`watchdog settlement should delay final drain, took ${elapsed}ms`,
				);
				assert.ok(
					elapsed < 6000,
					`settled watchdog should still allow cleanup, took ${elapsed}ms`,
				);
				assert.equal(result.exitCode, 0);
				assert.equal(result.finalOutput, "done-before-watchdog");
				assert.equal(
					(result as RunSyncResult & { watchdog?: { phase?: string } }).watchdog
						?.phase,
					"idle",
				);
			});
		...
		it("falls back after child watchdog tail timeout without failing successful foreground output", async () => {
			await withIsolatedWatchdogSettings(tempDir, async () => {
				...
				mockPi.onCall({
					jsonl: [
						events.assistantMessage("done-before-watchdog-timeout"),
						childWatchdogStatus("reviewing", 1),
					],
					keepAliveAfterFinalMessageMs: 10000,
				});
				...
				assert.ok(
					elapsed < 5000,
					`watchdog tail fallback should not hang, took ${elapsed}ms`,
				);
				assert.equal(result.exitCode, 0);
				assert.equal(result.finalOutput, "done-before-watchdog-timeout");
				const watchdog = (
					result as RunSyncResult & {
						watchdog?: { phase?: string; timedOut?: boolean };
					}
				).watchdog;
		...
		it("treats forced drain after final assistant output as cleanup success", async () => {
			...
			assert.ok(
				elapsed < 4000,
				`should clean up shortly after terminal stop, took ${elapsed}ms`,
			);
			assert.equal(result.exitCode, 0);
			assert.equal(result.error, undefined);
			assert.equal(result.finalOutput, "done-before-drain");
			assert.ok(
				!(result.progress?.recentOutput ?? []).some((line) =>
					line.includes("Forcing termination"),
				),
			);
		});
		...
		it("marks foreground runs that exceed timeoutMs as timed out", async () => {
			...
			assert.match(
				result.finalOutput ?? "",
				/Subagent timed out after 150ms\./,
			);
			assert.equal(result.progress.status, "failed");
		});
		...
		it("preserves a clean foreground completion after turn-budget work defers", async () => {
			...
			const result = await runSync(
				...
				agents,
				"worker",
				"Finish active tool work before enforcing the hard limit.",
				{
					turnBudget: { maxTurns: 1, graceTurns: 0 },
					...
					onUpdate(update: {
						details?: {
							...
							progress?: Array<{ currentTool?: string; status?: string }>;
						};
					}) {
			...
			const duringTool = snapshots.find(
				(snapshot) =>
					snapshot.turnBudget?.outcome === "termination-deferred" &&
					snapshot.currentTool === "bash",
			);
			assert.ok(
				duringTool,
				"expected a running snapshot with deferred termination and the active tool",
			);
			...
			assert.ok(
				result.messages?.some(
					(message) =>
						...
						JSON.stringify(message.content).includes("build completed"),
				),
			);
		});
		...
		it("does not run acceptance verification after a foreground timeout", async () => {
			...
			const report = [
				...
				JSON.stringify({
					...
					commandsRun: [
						{ command: "npm test", result: "passed", summary: "passed" },
					],
			...
			const runPromise = runSync(tempDir, agents, "echo", "Task", {
				...
				onUpdate: (update) => {
					...
					const sawCoordinationTool =
						...
						progress.some((p) => p?.currentTool === "contact_supervisor");
					if (!sawCoordinationTool) return;
					detachEmitted = true;
					eventBus.emit(SUBAGENT_DETACH_REQUEST_EVENT, {
		...
		it("enforces the stdout protocol limit after foreground detachment", async () => {
			...
			const result = await runSync(
				...
				makeAgentConfigs(["echo"]),
				"echo",
				"Detach then emit malformed output",
				{
					...
					onUpdate: (update) => {
						...
						if (
							!Array.isArray(progress) ||
							!progress.some(
								(item) => item.currentTool === "contact_supervisor",
							)
						)
							return;
						detachEmitted = true;
			...
			assert.equal(
				recoveredResult?.protocolError?.code,
		...
		it("finalizes explicit output before reporting detached child post-exit success", async () => {
			...
			for (
				let attempt = 0;
				attempt < 100 && (!fs.existsSync(outputPath) || !recoveredResult);
				attempt++
			) {
				await new Promise((resolve) => setTimeout(resolve, 20));
			}
		...
		it("lets an active supervisor child accept detach when another child is listening", async () => {
			...
			eventBus.on(SUBAGENT_DETACH_RESPONSE_EVENT, (payload) => {
				if (!payload || typeof payload !== "object") return;
				if (
					(payload as { requestId?: unknown }).requestId !== "parallel-request"
				)
					return;
				firstDetachResponse ??=
			...
			const [quietResult, supervisorResult] = await Promise.all([
				quietRun,
				supervisorRun,
			]);
