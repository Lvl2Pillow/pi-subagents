...
## Fork additions (Lvl2Pillow/pi-subagents)

This fork tracks upstream `nicobailon/pi-subagents` and adds one feature:

**Persistent subagents** (`alt+n`) — up to `slotCount` (default 3) long-lived background channels scoped per main-agent session. Each channel keeps its own context and transcript across pi restarts, streams live output into the chat, and supports scoped `/compact`, `/model`, `/name`, `/new`, and `/clone` commands. See `HANDOFF.md` for the full spec. Config lives under `persistentChat` in `config.json`.
