...
- Removed `systemPrompt` from `subagents.agentOverrides`; set a custom agent's system prompt in its md frontmatter/body or via the create/update actions.
- Removed the builtin subagent concept entirely. Agents are now only user, project, or package scoped: no bundled `agents/` directory, no `BUILTIN_AGENT_NAMES`, no builtin source in discovery or precedence, no `subagents.disableBuiltins` bulk-disable, and no builtin branches in eject/reset/admin/model flows. Override types and helpers were renamed from `BuiltinAgentOverride*` to `AgentOverride*`. Existing settings that mention now-removed builtin names or `disableBuiltins` are ignored as unknown keys.
...
- Removed the `subagents.disableThinking` setting and its builtin-only thinking-clearing logic; thinking now follows `defaultThinking`, frontmatter, and `agentOverrides` only.
