...
export type TurnBudgetOutcome =
	"within-budget" | "wrap-up-requested" | "termination-deferred" | "exceeded";
...
export type ProcessInstanceExitV1 =
	RunnerProcessInstanceExitV1 | PiWriterProcessInstanceExitV1;
...
export type SteeringActionState =
	"delivered" | "scheduled" | "pending" | "partial" | "recovered" | "failed";
...
export type PublicNestedRunSummary = Pick<
	NestedRunSummary,
	"id" | "parentRunId" | "parentStepIndex" | "parentAgent" | "depth" | "path" | "asyncDir" | "sessionId" | "sessionFile" | "ownerState" | "mode" | "state" | "agent" | "agents" | "model" | "thinking" | "currentStep" | "chainStepCount" | "parallelGroups" | "activityState" | "lastActivityAt" | "currentTool" | "currentToolStartedAt" | "currentPath" | "turnCount" | "toolCount" | "toolBudget" | "toolBudgetBlocked" | "totalTokens" | "totalCost" | "startedAt" | "endedAt" | "lastUpdate" | "error" | "timeoutMs" | "deadlineAt" | "timedOut" | "stopped" | "turnBudget" | "turnBudgetExceeded" | "wrapUpRequested"
> & {
...
export type AcceptanceLevel =
	"auto" | "none" | "attested" | "checked" | "verified";
...
export type AcceptanceInput =
	Exclude<AcceptanceLevel, "none"> | false | AcceptanceConfig;
...
export type AcceptanceLedgerStatus =
	AcceptanceEvidenceStatus | "review-required" | "reviewed" | "accepted";
...
export interface PersistentChatConfig {
	/**
	 * Keyboard shortcut that cycles the input target (main agent → persistent
	 * subagent 1 → … → main). Defaults to "alt+n" — a plain Option+letter
	 * chord that uses the legacy `\x1b<letter>` form. Works on any terminal
	 * that sends ESC for the meta/option key.
	 *
	 * Requires "Use Option as Meta key" on macOS Terminal.app
	 * (Settings → Profiles → Keyboard). iTerm2, kitty, wezterm, ghostty, and
	 * Alacritty send ESC for Option/Alt by default.
	 *
	 * Override here for a different key. Keys to avoid:
	 *   - ctrl+q, ctrl+s: tty flow control (XON/XOFF), may be swallowed
	 *   - alt+letter without Meta mode: produces a special char (alt+g → ©)
	 *   - ctrl+c/d/g/l/n/o/p/r/s/t/x/z: bound by pi or the editor
	 *   - alt+enter, alt+up, alt+down, alt+left, alt+right, alt+backspace,
	 *     alt+delete, alt+d, alt+b, alt+f, alt+y: bound by pi or the editor
	 *
	 * Examples: "alt+n", "alt+m", "alt+k", "ctrl+alt+n", "f5".
	 */
	switchShortcut?: string;
	/**
	 * Number of output lines shown in the collapsed subagent message.
	 * When the output is longer, the message ends with a dim
	 * "… (ctrl+o to expand)" line. Defaults to 1.
	 */
	maxCollapsedLines?: number;
	/**
	 * Number of always-present persistent subagent channels (1..8). Input
	 * cycles through them with the switch shortcut. Defaults to 3.
	 */
	slotCount?: number;
}
