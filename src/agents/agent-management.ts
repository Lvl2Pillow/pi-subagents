...
function allAgents(d: {
	...
	return [...d.package, ...d.user, ...d.project];
}
...
function findAgents(
	...
	const scoped = mergeAgentsForScope(
		scope,
		d.user,
		d.project,
		d.package,
	...
	if (resolved.agent)
		return scoped
			.filter((agent) => agent.name === resolved.agent!.name)
			.sort((a, b) => a.source.localeCompare(b.source));
	return scoped
		.filter(
			(agent) =>
				Boolean(resolveAgentName(raw, [agent]).agent) ||
				(sanitized !== raw &&
					Boolean(resolveAgentName(sanitized, [agent]).agent)),
		)
		.sort((a, b) => a.source.localeCompare(b.source));
}
...
function findChains(
	...
	return discoverAgentsAll(cwd)
		.chains.filter(
			(c) =>
				(scope === "both" || c.source === "package" || c.source === scope) &&
...
// Returns the highest-precedence definition for a resolved canonical name (project > user > package),
...
function resolveEffectiveAgent(
	...
	const matches = candidates.filter(
		(agent) => agent.name === resolved.agent!.name,
	);
	return {
		agent: matches.reduce((best, agent) =>
			AGENT_SOURCE_PRECEDENCE[agent.source] >
			AGENT_SOURCE_PRECEDENCE[best.source]
				? agent
				: best,
		),
	};
	...
	return [
		...new Set(
			steps
				.map((s) => s.agent)
				.filter(
					(agentName): agentName is string =>
						typeof agentName === "string" &&
						!resolveAgentName(agentName, agents).agent,
				),
		),
...
export function preservedAgentFrontmatterFields(
	agent: AgentConfig,
...
function resolveTarget<
	T extends { name: string; source: AgentSource; filePath: string },
>(
	kind: "agent" | "chain",
	name: string,
	matches: T[],
	...
	if (distinctNames.length > 1) {
		return result(
			`Ambiguous ${kind} alias or name '${name}': ${distinctNames.sort((a, b) => a.localeCompare(b)).join(", ")}`,
			true,
		);
	}
...
function handleModels(
	...
	if (requestedAgent) {
		...
		if (!agent)
			return result(
				`Agent '${requestedAgent}' not found. Available: ${[...byName.keys()].join(", ") || "none"}.`,
				true,
			);
...
function handleGet(
	...
	if (!scope)
		return result(
			"agentScope must be 'user', 'project', or 'both' for get.",
			true,
		);
...
function handleEject(
	...
	const d = discoverAgentsAll(ctx.cwd);
	const source = [...d.package].find(
		(a) => a.name === raw || a.name === sanitized,
	);
	if (!source) {
		return result(
			`Agent '${raw}' not found or is not a package agent. eject copies a package agent to ${scope} scope so it can be customized. Available: ${availableNames(ctx.cwd, "agent").join(", ") || "none"}.`,
			true,
		);
	}
...
function handleDisable(
	...
	const settingsPath = mergeAgentOverride(ctx.cwd, runtimeName, scope, {
		disabled: true,
	});
...
function handleEnable(
	...
	const runtimeName = effective.agent.name;
	const { path: settingsPath, removed } = removeAgentOverrideFields(
		ctx.cwd,
		runtimeName,
		scope,
		["disabled"],
	...
	return result(
		`Agent '${runtimeName}' is still disabled after removing the ${scope} disabled override. It may declare disabled: true in its own agent definition.`,
		true,
	);
}
...
function handleReset(
	...
	const overrideRemoval = removeAgentOverride(
		ctx.cwd,
		runtimeName,
		scope,
	);
