...
/** Stable per-session supervisor target identifier for bridge activation and instructions. */
...
function buildIntercomBridgeInstruction(orchestratorTarget: string | undefined, template: string): string {
	const instruction = template.replaceAll("{orchestratorTarget}", orchestratorTarget || "the supervisor session").trim();
